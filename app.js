...
function openModal(title, body, actionsHtml=''){
  ...
  if(!safeBody){ showToast('Não existe conteúdo disponível para esta ação.'); return; }
  setText('modalTitle', safeTitle);
  document.getElementById('modalBody').innerHTML = safeBody;
  document.getElementById('modalActions').innerHTML = actionsHtml || '<button class="btn btn-primary" type="button" data-close-modal>Fechar</button>';
  document.getElementById('modalRoot').hidden = false;
  document.querySelectorAll('[data-close-modal]').forEach(b=>b.addEventListener('click', closeModal));
  document.querySelectorAll('[data-form-submit]').forEach(b=>b.addEventListener('click', () => submitForm(b.dataset.formSubmit)));
}
function closeModal(){ const m=document.getElementById('modalRoot'); if(m) m.hidden=true; }
